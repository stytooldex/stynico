...
//点击Context菜单的项时发生
	...
	{
	    item = new HashMap<String, Object>();  
            item.put("imageItem", R.drawable.image);  
	...
	files_name = new ArrayList<String>();
        dirs_name = new ArrayList<String>();
        ...
        for (int i=0;i < files.length;i++)
	{
	    ...
	    {
		dirs_name.add(files[i].getName() + "");
	    }
	    //文件符合，添加文件名
	    else if (files[i].isFile() && files[i].getName().matches(filter))
	    {
		files_name.add(files[i].getName());
	    }
        }
	//对数组进行排列
