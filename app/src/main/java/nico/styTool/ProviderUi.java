...
public class ProviderUi extends AppCompatActivity
{
    int type=0;
    String path="";//视频路径
    ...
    private void muxerAudio(String file_in, String file_out)
    {
		...
		try
		{
			...
			while (true)
			{
				...
				count += stampTime;
				handler.obtainMessage(1, count / 100 + "/" + size + "  " + Math.round(count / 10f / size) + "%").sendToTarget();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
        ...
        if (requestCode == 1)
		{
            if (resultCode == RESULT_OK)
			{
				...
				handler.obtainMessage(200, path).sendToTarget();
				//
			}
        }
    }
