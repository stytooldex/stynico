...
public class buff_ext extends AppCompatActivity 
implements AdapterView.OnItemClickListener, IUninstall, SearchView.OnQueryTextListener, AdapterView.OnItemLongClickListener
{
	...
	apitils.KEY = query.trim();
	list = apitils.getSearchResult(allList, query);//根据关键字生成结果
	update_sort();//重新排序事件
	...
	apitils.KEY = newText.trim();
	list = apitils.getSearchResult(allList, newText.trim());//根就关键字生成结果
	update_sort();//重新排序更新
